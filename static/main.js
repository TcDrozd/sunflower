...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.showPhotoModal = function(photoId) {
    fetch(`/photo/${photoId}`)
      ...
      .then(photo => {
        const title = photo.date_taken ? `Photo from ${photo.date_taken}` : `Photo: ${photo.original_filename}`;
        ...
        const content = `
          ...
          <div style="margin-bottom: 1rem;">
            ...
            <p><strong>📤 Uploaded:</strong> ${new Date(photo.upload_date).toLocaleDateString()}</p>
          </div>
          ${cameraInfo}
        `;
      ...
      .catch(error => {
        ...
        const bodyEl = document.getElementById('photoModalBody');
        if (bodyEl) bodyEl.innerHTML = '<p>Error loading photo details.</p>';
        showModal('photoModal');
      });
  };
